...
namespace API.Extensions
{
    public static class ClaimsPrincipalExtensions
    {
        ...
        {
            var userToReturn = await userManager.Users.SingleOrDefaultAsync(x => x.Email == user.GetEmail());
        ...
        {
            var userToReturn = await userManager.Users.Include(x => x.Address).SingleOrDefaultAsync(x => x.Email == user.GetEmail());
            if (userToReturn is null) throw new AuthenticationException("User not found");
            return userToReturn;
        }
